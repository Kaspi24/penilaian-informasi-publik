<x-app-layout x-data="{
        showaddAdminModal: {{ $errors->hasBag('create_admin') ? 'true' : 'false' }},

        showEditAdminModal: {{ $errors->hasBag('edit_admin') ? 'true' : 'false' }},
        edit_id         : '{{ $errors->hasBag('edit_admin') ? old('id') : '' }}',
        edit_name       : '{{ $errors->hasBag('edit_admin') ? old('name') : '' }}',
        edit_username   : '{{ $errors->hasBag('edit_admin') ? old('username') : '' }}',
        edit_email      : '{{ $errors->hasBag('edit_admin') ? old('email') : '' }}',
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-sm mb-6">
                <div class="p-4 text-primary flex justify-between items-center">
                    <p class="text-lg lg:text-xl font-bold">Data Admin Terdaftar</p>
                    <button type="button" x-on:click="showaddAdminModal = true" class="w-fit aspect-square md:aspect-auto p-1 md:p-2 md:pr-4 rounded bg-emerald-600 text-white md:flex md:gap-2 md:items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="size-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                        </svg>
                        <p class="hidden md:inline-block text-xs font-black uppercase">Tambah Admin</p>
                    </button>
                </div>
            </div>
    ...
    <x-slot name="modals">
        <!-- AddAdminModal -->
        <div class="fixed z-[2220] inset-0" x-cloak x-show="showaddAdminModal">
            <div class="absolute z-[2222] inset-0 bg-primary-90 bg-opacity-30 flex justify-center items-center py-4">
                <div class="bg-white w-10/12 md:w-1/2 lg:w-1/3 rounded-md p-5 lg:p-6 flex flex-col justify-center items-center">
                    ...
                    <form action="{{ route('admin.store') }}" method="POST" class="w-full">
                        @csrf
                        <div class="mb-4">
                            ...
                            <x-input-error class="mt-2" :messages="$errors->create_admin->first('name')" />
                            {{-- <x-input-error class="mt-2" :messages="$errors->get('name', 'create_admin')" /> --}}
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <x-input-error class="mt-2" :messages="$errors->create_admin->first('username')" />
                            {{-- <x-input-error class="mt-2" :messages="$errors->get('username', 'create_admin')" /> --}}
                        </div>
                
                        <div class="mb-4">
                            <x-input-label for="email" :value="__('Email')" />
                            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email')" required autocomplete="off" />
                            <x-input-error class="mt-2" :messages="$errors->create_admin->first('email')" />
                            {{-- <x-input-error class="mt-2" :messages="$errors->get('email', 'create_admin')" /> --}}
                        </div>

                        <div class="mb-4">
                            <x-input-label for="password" :value="__('Password ')" />
                            <x-text-input id="password" name="password" type="password" class="mt-1 block w-full" autocomplete="new-password" />
                            <x-input-error :messages="$errors->create_admin->first('password')" class="mt-2" />
                            {{-- <x-input-error :messages="$errors->get('password', 'create_admin')" class="mt-2" /> --}}
                        </div>
                
                        <div class="mb-8">
                            <x-input-label for="password_confirmation" :value="__('Konfirmasi Password')" />
                            <x-text-input id="password_confirmation" name="password_confirmation" type="password" class="mt-1 block w-full" autocomplete="new-password" />
                            <x-input-error :messages="$errors->create_admin->first('password_confirmation')" class="mt-2" />
                            {{-- <x-input-error :messages="$errors->get('password_confirmation', 'create_admin')" class="mt-2" /> --}}
                        </div>
                        
                        <div class="w-full flex justify-center items-center gap-4">
                            <button type="button" x-on:click="showaddAdminModal = false" class="block w-[48%] text-primary bg-white border border-primary focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium hover:font-semibold rounded-md text-sm py-2 text-center">
                                KEMBALI
                            </button>
        ...
        <!-- EditAdminModal -->
        <div class="fixed z-[2220] inset-0" x-cloak x-show="showEditAdminModal">
            <div class="absolute z-[2222] inset-0 bg-primary-90 bg-opacity-30 flex justify-center items-center py-4">
                <div class="bg-white w-10/12 md:w-1/2 lg:w-1/3 rounded-md p-5 lg:p-6 flex flex-col justify-center items-center">
                    <div class="w-full text-center mb-3">
                        <p class="text-lg lg:text-xl text-primary font-bold tracking-wide mb-2 uppercase">
                    ...
                    <form action="{{ route('admin.update') }}" method="POST" class="w-full">
                        @csrf @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <x-input-error class="mt-2" :messages="$errors->edit_admin->first('name')" />
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <x-input-error class="mt-2" :messages="$errors->edit_admin->first('username')" />
                        </div>
                
                        <div class="mb-4">
                            <x-input-label for="email" :value="__('Email')" />
                            <x-text-input id="edit_email" name="email" type="email" class="mt-1 block w-full" x-model="edit_email" required autocomplete="off" />
                            <x-input-error class="mt-2" :messages="$errors->edit_admin->first('email')" />
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <x-input-error :messages="$errors->edit_admin->first('password')" class="mt-2" />
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <x-input-error :messages="$errors->edit_admin->first('password_confirmation')" class="mt-2" />
                        </div>
        ...
        <!-- DeleteAdminModal -->
        <div class="fixed z-[2220] inset-0" x-cloak x-show="showDeleteAdminModal">
            <div class="absolute z-[2222] inset-0 bg-primary-90 bg-opacity-30 flex justify-center items-center py-4">
                <div class="bg-white w-10/12 md:w-1/2 lg:w-1/3 rounded-md p-5 lg:p-6 flex flex-col justify-center items-center">
                    <div class="w-full text-center mb-3">
                        <p class="text-lg lg:text-xl text-primary font-bold tracking-wide mb-2 uppercase">
                    ...
                    <form action="{{ route('admin.delete') }}" method="POST" class="w-full">
                        @csrf @method('DELETE')
                        ...
                        <div class="w-full bg-danger-10/25 p-3 rounded border border-danger mb-3">
                            <p>Anda yakin ingin menghapus data Admin berikut :</p>
                            <table>
