@php
    $response       = \App\Models\RespondentScore::firstWhere('respondent_id',$row->id);
    $respondent     = \App\Models\User::firstWhere('id',$row->id);
    $respondent->load('answers');
    // $assigned_count = $assigned->count();
    // $finished_count = $assigned->where('is_done_scoring', true)->count();
    // $respondent     = \App\Models\User::find($row->id);
    // dump($respondent);
@endphp
