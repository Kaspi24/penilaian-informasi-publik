<div class="flex w-40 items-center justify-between">
    @if ($row->score->is_published)
        <button type="button" class="w-full py-1.5 pl-2 pr-3 rounded-md text-xs font-extrabold tracking-tight bg-emerald-800  text-white flex gap-1.5 items-center justify-center" disabled>
            <span>
    ...
    @else
        @if (!$row->score->is_done_scoring)
            @if ($row->score->is_done_filling)
                <a href="{{ route('questionnaire.evaluate',$row->id) }}" target="_blank" class="w-full py-1.5 pl-2 pr-3 rounded-md text-xs font-extrabold tracking-tight bg-emerald-600 hover:bg-emerald-700 text-white flex gap-1.5 items-center justify-center">
                    <span>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4">
